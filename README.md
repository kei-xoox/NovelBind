...
### 💡 表紙画像の設定
EPUBに表紙を付けたい場合は、実行前にプロジェクトのルートディレクトリ（main.py と同じ場所）に cover.jpg という名前で画像ファイルを配置してください。 スクリプトが自動的に検出し、EPUBの表紙として設定します。

* 推奨サイズ: 1600 x 2560 ピクセル (アスペクト比 1:1.6)
* 対応形式: JPEG
...
- `cover.jpg`: (オプション) EPUBの表紙に使用される画像ファイル。
